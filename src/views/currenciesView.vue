<template>
    <div class="currencies-container">
        <h2 class="title">Gestión de Monedas</h2>

        <!-- Botón para abrir el modal -->
        <div class="btn-container">
            <button class="btn-agregar" @click="abrirModal()">Nueva Moneda</button>
        </div>
        ...
        <div class="tabla-container">
            <table class="tabla">
                ...
                <tbody>
                    <tr v-for="currency in currenciesStore.currencies" :key="currency.id">
                        <td>{{ currency.id }}</td>
                        <td>{{ currency.name }}</td>
                        <td>
                            <button class="btn-accion editar" @click="abrirModal(currency)">Editar</button>
                            <button class="btn-accion eliminar" @click="eliminar(currency.id)">Eliminar</button>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div v-if="mostrarModal" class="overlay" @click.self="cerrarModal">
            <div class="modal-contenido neon-card">
                <div class="modal-header">
                    <h3>{{ modoEdicion ? 'Editar Moneda' : 'Nueva Moneda' }}</h3>
...
function abrirModal(currency = null) {
    modoEdicion.value = !!currency
    form.value = currency
...
async function guardar() {
    ...
    try {
        if (modoEdicion.value) {
            await currenciesStore.update(form.value)
        } else {
            await currenciesStore.create(form.value)
        }
        cerrarModal()
    } catch (err) {
        console.error(err)
...
async function eliminar(id) {
    if (confirm('¿Seguro que deseas eliminar esta moneda?')) {
        try {
            await currenciesStore.delete(id)
        } catch (err) {
            console.error(err)
        }
    }
