...
<template>
  <div class="min-h-screen bg-secondary flex overflow-hidden font-sans relative">
    ...
    <div
      class="hidden lg:flex lg:w-1/3 relative flex-col justify-between p-16 overflow-hidden bg-[#0a0b0d] border-r border-white/5"
    >
      <div
        class="absolute top-[-20%] left-[-10%] w-[80%] h-[80%] bg-success/10 blur-[120px] rounded-full animate-pulse"
      ></div>
      <div
      ...
      <div class="relative z-10">
        <div class="flex items-center gap-4 mb-20">
          <div
            class="w-12 h-12 bg-white rounded-xl flex items-center justify-center p-1 overflow-hidden shadow-xl"
          >
        ...
        <div class="max-w-xs">
          <div
            class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-success/10 border border-success/20 mb-6"
          >
            <span class="w-1.5 h-1.5 rounded-full bg-success animate-pulse"></span>
            <span class="text-[0.55rem] font-black text-success uppercase tracking-[0.2em]"
              >Registro Abierto</span
            >
          </div>
          ...
          <p class="text-white/30 text-sm font-medium leading-relaxed">
            Inicia hoy la transformación de tu negocio. Gestión multi-cliente, auditoría en tiempo
            real y seguridad bancaria unificada.
          </p>
        </div>
      </div>
      ...
      <div class="relative z-10 flex flex-col gap-6">
        <div
          class="flex items-center gap-4 text-white/20 text-[0.55rem] font-black uppercase tracking-[0.4em]"
        >
    ...
    <div class="w-full lg:w-2/3 flex flex-col bg-secondary relative">
      ...
      <div
        class="p-8 lg:px-16 lg:py-10 flex justify-between items-center border-b border-white/5 bg-secondary/50 backdrop-blur-xl sticky top-0 z-20"
      >
        <div class="flex items-center gap-4 lg:hidden">
          <div
            class="w-10 h-10 bg-white rounded-lg flex items-center justify-center p-1 overflow-hidden"
          >
        ...
        <div class="hidden lg:block">
          ...
          <p class="text-white/20 text-[0.6rem] font-black uppercase tracking-[0.2em]">
        ...
        <button
          @click="goToLogin"
          class="text-white/40 hover:text-primary transition-colors text-[0.6rem] font-black uppercase tracking-[0.2em] flex items-center gap-2 group"
        >
          ¿Ya tienes cuenta?
          <span
            class="text-primary group-hover:underline decoration-2 underline-offset-4 font-black"
            >Iniciar Acceso</span
          >
        </button>
      </div>
          ...
          <form @submit.prevent="handleRegister" class="space-y-16 pb-20">
            ...
            <section class="space-y-8">
              <div class="flex items-center gap-4">
                <span class="text-[0.6rem] font-black text-primary/40 uppercase tracking-[0.4em]"
                  >01. Identidad Fiscal</span
                >
              ...
              <div class="space-y-4">
                <label
                  class="block text-[0.6rem] font-black text-white/20 uppercase tracking-[0.3em] ml-1"
                  >Razón o Nombre Comercial</label
                >
                <div class="relative group">
                  <div
                    class="absolute left-6 top-1/2 -translate-y-1/2 text-white/10 group-focus-within:text-primary transition-all duration-500"
                  >
                ...
                <span
                  v-if="getError('company_name')"
                  class="text-danger text-[0.65rem] ml-1 font-bold"
                  >{{ getError('company_name') }}</span
                >
              </div>
            </section>
            ...
            <section class="space-y-8">
              <div class="flex items-center gap-4">
                <span class="text-[0.6rem] font-black text-info/40 uppercase tracking-[0.4em]"
                  >02. Master Admin</span
                >
                <div class="h-px bg-white/5 grow"></div>
              </div>
            ...
            <section class="space-y-8">
              <div class="flex items-center gap-4">
                <span class="text-[0.6rem] font-black text-success/40 uppercase tracking-[0.4em]"
                  >03. Nivel de Alcance</span
                >
                <div class="h-px bg-white/5 grow"></div>
              </div>
            ...
            <div class="pt-8 flex flex-col items-center gap-8">
              <label class="flex items-start gap-4 cursor-pointer group max-w-md">
                <input
                  ...
                  class="mt-1 w-6 h-6 rounded-lg bg-white/5 border-white/10 accent-primary text-primary"
                />
                <span
                  class="text-[0.65rem] font-bold text-white/30 uppercase tracking-tight leading-relaxed group-hover:text-white/50 transition-colors"
                >
                  Acepto el
                  <a @click.prevent="showTermsModal = true" class="text-primary hover:underline"
                    >Aviso de Privacidad</a
                  >
                  y los protocolos de seguridad industrial de TuConpay.
                </span>
              </label>
              ...
              >
                ...
                <template v-else
                  ><div
                    class="w-5 h-5 border-2 border-secondary/20 border-t-secondary rounded-full animate-spin"
                  ></div
                ></template>
              </BaseButton>
              ...
              <div v-if="showPayPal" class="w-full space-y-4 animate-premium-in">
                <div class="p-4 bg-primary/5 border border-primary/20 rounded-2xl mb-4 text-center">
                  <p class="text-[0.6rem] font-black text-primary uppercase tracking-[0.2em]">
                    Pagar Plan {{ form.plan.toUpperCase() }}
                  </p>
                </div>
                <PayPalButton
                  ...
                  :token="registrationData?.access_token"
                  @success="handlePaymentSuccess"
                />
                <button
                  @click="showPayPal = false"
                  class="text-white/20 hover:text-white text-[0.55rem] font-black uppercase tracking-[0.2em] transition-colors"
                >
    ...
    >
      <div class="space-y-6 text-white/70 text-sm leading-relaxed px-2">
        <p class="italic text-white/30 text-[0.6rem] uppercase tracking-widest">
          Última actualización: 12 de marzo de 2026
        </p>

        <div class="space-y-4">
          <p>
            En <strong class="text-white">Productos Digitales SyO LLC</strong>, valoramos la
            privacidad de nuestros usuarios. Esta Política de Privacidad describe cómo tratamos la
            información en la aplicación <strong class="text-primary">Tu Conpay</strong>.
          </p>
          <p>
        ...
        <div class="space-y-4 py-4 border-t border-white/5">
          <h4 class="text-primary font-black uppercase tracking-[0.2em] text-[0.65rem]">
          ...
          <ul class="space-y-3">
            <li class="flex gap-3">
              ...
              <span
                ><strong class="text-white">Datos de Cuenta:</strong> Información básica para la
                creación del perfil (nombre, correo electrónico y datos de autenticación).</span
              >
            </li>
            <li class="flex gap-3">
              <span class="text-primary mt-1">•</span>
              <span
                ><strong class="text-white">Datos de Transacciones:</strong> Información que el
                Usuario introduce manualmente sobre sus operaciones de corretaje o P2P (montos,
                divisas, nombres de contrapartes, fechas).</span
              >
            </li>
          </ul>
        </div>
        ...
        <div class="space-y-4 py-4 border-t border-white/5">
          ...
          <p>
            Nuestra filosofía es de acceso mínimo. La información se utiliza exclusivamente para:
          </p>
          <ul class="space-y-3">
            <li class="flex gap-3">
              <span class="text-primary">•</span>
              <span>Proporcionar las funciones de registro solicitadas.</span>
            </li>
            <li class="flex gap-3">
              <span class="text-primary">•</span>
              <span>Generar reportes y estadísticas privadas.</span>
            </li>
            <li class="flex gap-3">
              <span class="text-primary">•</span>
              <span>Brindar soporte técnico bajo petición expresa.</span>
            </li>
          </ul>

          <div
            class="bg-primary/5 p-6 rounded-2xl border border-primary/20 mt-6 group transition-all hover:bg-primary/10"
          >
            <p class="text-primary text-[0.7rem] font-bold leading-relaxed">
              <strong
                class="uppercase tracking-widest block mb-2 underline decoration-2 underline-offset-4"
                >Garantía de Privacidad:</strong
              >
              Productos Digitales SyO LLC NO vende, NO alquila, NO intercambia ni utiliza los datos
              de sus transacciones para fines publicitarios o de marketing. Sus datos le pertenecen
        ...
        <div class="space-y-4 py-4 border-t border-white/5">
          <h4 class="text-primary font-black uppercase tracking-[0.2em] text-[0.65rem]">
            03. Seguridad de Nivel Bancario
          </h4>
          ...
          <ul class="space-y-3">
            <li class="flex gap-3">
              <span class="text-primary mt-1">•</span>
              <span
                ><strong class="text-white">Cifrado:</strong> Datos cifrados en tránsito (SSL/TLS) y
                en reposo.</span
              >
            </li>
            <li class="flex gap-3">
              <span class="text-primary mt-1">•</span>
              <span
                ><strong class="text-white">Acceso Restringido:</strong> Solo personal autorizado
                tiene acceso limitado bajo petición.</span
              >
            </li>
          </ul>
        </div>
        ...
        <div class="space-y-4 py-4 border-t border-white/5">
          <h4 class="text-primary font-black uppercase tracking-[0.2em] text-[0.65rem]">
            04. Control del Usuario
          </h4>
          <p>Usted es el dueño de su información en todo momento:</p>
          <ul class="space-y-3 flex flex-wrap gap-2">
            <li
              class="bg-white/5 px-4 py-2 rounded-xl text-white font-bold text-[0.6rem] uppercase tracking-wider"
            >
              Acceder y Exportar
            </li>
            <li
      ...
      <template #footer>
        <BaseButton
          ...
          class="!w-full !h-12 !rounded-xl !text-xs !uppercase !tracking-widest"
